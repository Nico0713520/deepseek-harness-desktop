...
export const ABILITIES: readonly AbilityDefinition[] = [
  ...
  ability({
    ...
    title: 'Playwright 网页自动化',
    outcome: '让 DeepSeek 帮你操作浏览器、做网页测试，或搭建可复查的爬虫流程。',
    summary: '从一个具体网页任务开始，先限定网址、动作和数据范围，再生成可运行、可停止的自动化脚本。',
    industryIds: ['programmer'],
    kindIds: ['coding'],
    collectionIds: ['vibe-coding'],
    aliases: ['Playwright', '浏览器自动化', '网页自动化', '爬虫', '网页测试', 'browser automation'],
    examples: ['抓取公开网页中的商品信息', '给登录后的后台写一条自动化测试'],
    userProvides: '目标网址、允许访问的页面、需要提取或验证的内容和停止条件。',
    userReceives: '一个可运行的浏览器自动化方案、启动命令、样例结果和停止方法。',
    suitableFor: '你想做网页测试、公开资料采集或重复的浏览器操作。',
    readsOrChanges: '只访问你明确允许的网页；默认不提交表单、不绕过验证、不修改远程数据。',
    rollback: '停止脚本并删除生成的项目目录或测试文件即可，浏览器数据单独保留。',
    estimatedTime: '约 15–30 分钟',
    implementation: {
      extensionTypes: ['plugin', 'skill'],
      goal: '帮我用 Playwright 创建一个浏览器自动化能力。先确认目标网址、允许的页面、读取或点击范围和停止条件；优先做只读演示，不绕过登录验证、不提交远程表单，生成后提供启动、验证和停止方法。',
      checks: ['只访问明确允许的页面和数据', '用一次小范围样例验证选择器和输出', '说明如何停止、清理和扩大范围'],
